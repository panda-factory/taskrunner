...
#include "delayed_task_queues_impl.h"
...
DelayedTaskQueuesImpl *DelayedTaskQueuesImpl::GetInstance()
{
    static DelayedTaskQueuesImpl *instance = nullptr;
    if (!instance) {
        instance = new DelayedTaskQueuesImpl();
    }
    return instance;
}

void DelayedTaskQueuesImpl::AddTaskObserver(TaskQueueId queue_id,
                                            intptr_t key,
...
void DelayedTaskQueuesImpl::RemoveTaskObserver(TaskQueueId queue_id,
                                               intptr_t key)
{
    std::scoped_lock locker(queue_mutex_);
...
TaskQueueId DelayedTaskQueuesImpl::CreateTaskQueue()
{
    std::scoped_lock locker(queue_mutex_);
...
void DelayedTaskQueuesImpl::Dispose(TaskQueueId queue_id)
{
    std::scoped_lock locker(queue_mutex_);
...
void DelayedTaskQueuesImpl::DisposeTasks(TaskQueueId queue_id)
{
    std::scoped_lock locker(queue_mutex_);
    const auto &queue_entry = queue_entries_.at(queue_id);
    queue_entry->Erase();
...
std::vector<std::function<void ()>> DelayedTaskQueuesImpl::GetObserversToNotify(
...
std::function<void ()> DelayedTaskQueuesImpl::GetNextTask(TaskQueueId queue_id,
...
std::chrono::steady_clock::time_point DelayedTaskQueuesImpl::GetNextWakeTimeUnlocked(
        TaskQueueId queue_id) const
{
    return PeekNextTaskUnlocked(queue_id).GetTargetTime();
}

size_t DelayedTaskQueuesImpl::GetNumPendingTasks(TaskQueueId queue_id) const
{
    std::scoped_lock locker(queue_mutex_);
    const auto& queue_entry = queue_entries_.at(queue_id);
...
bool DelayedTaskQueuesImpl::HasPendingTasks(
        TaskQueueId queue_id) const
{
...
wtf::DelayedTask DelayedTaskQueuesImpl::PeekNextTaskUnlocked(
...
void DelayedTaskQueuesImpl::RegisterTask(
...
void DelayedTaskQueuesImpl::SetWakeable(TaskQueueId queue_id,
...
void DelayedTaskQueuesImpl::WakeUpUnlocked(TaskQueueId queue_id,
